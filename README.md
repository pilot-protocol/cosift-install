...
exact per-harness plan for your machine printed without a single byte being written.
...
recovered rather than minted), which harnesses were configured, an `onboarded` flag, and the
install timestamp. It does not contain the token.
...
An unrecognised option prints usage to stderr and exits `2`.
...
`--uninstall` is documented in [docs/UNINSTALL.md](docs/UNINSTALL.md).
